...
void WRITE_TO_SERVER(int sockfd, int msg)
{
...
void draw_PLACE_BORAD(char PLACE_BORAD[][3])
{
    printf(" %c | %c | %c \n", PLACE_BORAD[0][0], PLACE_BORAD[0][1], PLACE_BORAD[0][2]);
    printf("-----------\n");
    printf(" %c | %c | %c \n", PLACE_BORAD[1][0], PLACE_BORAD[1][1], PLACE_BORAD[1][2]);
    printf("-----------\n");
    printf(" %c | %c | %c \n", PLACE_BORAD[2][0], PLACE_BORAD[2][1], PLACE_BORAD[2][2]);
}
...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        else if (!strcmp(msg, INVALID_CMD)) { /* Move was invalid. Note that a "TRN" message will always follow an "INV" message, so we will end up at the above case in the next iteration. */
            printf("That position has already been played. Try again.\n");
        }
        else if (!strcmp(msg, COUNT_CMD)) { /* Server is sending the number of active players. Note that a "TRN" message will always follow a "CNT" message. */
            int num_players = recv_int(sockfd);
            printf("There are currently %d active players.\n", num_players);
        }
        else if (!strcmp(msg, UPDATE_CMD)) { /* Server is sending a game PLACE_BORAD update. */
            UPDATE_GET(sockfd, PLACE_BORAD);
            draw_PLACE_BORAD(PLACE_BORAD);
        }
        else if (!strcmp(msg, WAIT_CMD)) { /* Wait for other player to take a turn. */
            printf("Waiting for other players move...\n");
        }
        else if (!strcmp(msg, WIN_CMD)) { /* Winner. */
            printf("You win!\n");
            break;
        }
        else if (!strcmp(msg, LOSE_CMD)) { /* Loser. */
            printf("You lost.\n");
            break;
        }
        else if (!strcmp(msg, DRAW_CMD)) { /* Game is a draw. */
            printf("Draw.\n");
            break;
        }
